...
def make_solid_file(nx, ny, bottom_val, side_val, top_val1, top_val2, top_val3, reservoir_value, latsize, zdepth, river_dir, root_name, out_dir, pftools_path='./'):
    ...
    if river_dir == 1:
        ...
        patch[int(np.floor(ny/2)), int(np.floor(nx/2))] = reservoir_value
    ...
    # run the pfmask-to-pfsol tool#top
    #     top = patch.reshape(nx*ny)
    #     file = os.path.join(out_dir, (root_name+"_top.asc"))
    #     np.savetxt(file, top, fmt='%i', header=header, comments="")
